...
from TorchTrainer.utils.dl_utils import tensor2imgs
...
class NaiveVisualizationHook(Hook):
    ...
    def __init__(self, interval: int = 1, draw_gt: bool = True, draw_pred: bool = True):
        self.draw_gt = draw_gt
        self.draw_pred = draw_pred
        self._interval = interval

    def _unpad(self, input: np.ndarray, unpad_shape: Tuple[int, int]) -> np.ndarray:
    ...
    def after_test_iter(
        ...
        if self.every_n_inner_iters(batch_idx, self._interval):
            for data, output in zip(data_batch, outputs):
                ...
                input = tensor2imgs(input, **data_sample.get("img_norm_cfg", dict()))[0]
                ...
                if "pad_shape" in data_sample:
                    input = self._unpad(input, data_sample.get("scale", ori_shape))
