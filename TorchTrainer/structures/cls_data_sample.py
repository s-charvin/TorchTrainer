...
import numpy as np
import torch
from TorchTrainer.structures import BaseDataElement, LabelData
...
class ClsDataSample(BaseDataElement):
    """用于管理分类样本数据的 gt_label, pred_label. 数据类型为 LabelData, 其中包含了 label 和 score 数据, 以及 id <-> label 的转换方法.
    Args:
        BaseDataElement (_type_): _description_
    """
    def set_gt_label(
