...
from TorchTrainer.utils.dl_utils import load_url

# `TorchTrainer_HOME` is the highest priority directory to save checkpoints
...
ENV_TorchTrainer_HOME = "TorchTrainer_HOME"
...
def _get_TorchTrainer_home():
    ...
    mkdir_or_exist(TorchTrainer_home)
    return TorchTrainer_home
...
def get_torchvision_models():
    ...
    if digit_version(torchvision.__version__) < digit_version("0.13.0a0"):
        ...
        json_path = osp.join(TorchTrainer.__path__[0], "hub/torchvision_0.12.json")
        model_urls = TorchTrainer.load(json_path)
...
def get_external_models():
    TorchTrainer_home = _get_TorchTrainer_home()
    default_json_path = osp.join(TorchTrainer.__path__[0], "hub/openmmlab.json")
    default_urls = load_file(default_json_path)
    assert isinstance(default_urls, dict)
    external_json_path = osp.join(TorchTrainer_home, "open_mmlab.json")
...
def get_mmcls_models():
    mmcls_json_path = osp.join(TorchTrainer.__path__[0], "hub/mmcls.json")
...
def get_deprecated_model_names():
    deprecate_json_path = osp.join(TorchTrainer.__path__[0], "hub/deprecated.json")
...
def load_from_openmmlab(filename, map_location=None):
    ...
    if model_url.startswith(("http://", "https://")):
        ...
        filename = osp.join(_get_TorchTrainer_home(), model_url)
        if not osp.isfile(filename):
            raise FileNotFoundError(f"{filename} can not be found.")
        checkpoint = torch.load(filename, map_location=map_location)
    return checkpoint
