...
from TorchTrainer.utils.registry import METRICS
...
class DumpResults(BaseMetric):
    """将模型预测结果转储到 pickle 文件以进行离线评估.

    Args:
        out_file_path (str): 转储文件的路径. 必须以'.pkl'或'.pickle'结尾.
        collect_device (str): 用于在分布式训练期间从不同的 rank 收集结果的设备名称. 必须是'cpu'或'gpu'. Defaults to 'cpu'.
        collect_dir: (str, optional): 用于从不同 rank 收集数据的同步目录. 此参数仅在 ``collect_device`` 为 'cpu' 时配置. Defaults to None.
    """
...
def _precision_recall_f1_support(pred_positive, gt_positive, average):
    ...
    # 忽略 -1 目标, 例如不需要评估结果的困难样本, 仅用于计算多标签而不影响单标签行为
