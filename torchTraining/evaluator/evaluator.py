...
from torchTraining.datasets import pseudo_collate,input_collate
from torchTraining.utils.registry import EVALUATOR, METRICS
...
class Evaluator:
    ...
    def offline_evaluate(self,
        ...
        for output_chunk in get_chunks(iter(data_samples), chunk_size):
            if data is not None:
                data_chunk = pseudo_collate(next(data)) # input_collate
            else:
                data_chunk = None
