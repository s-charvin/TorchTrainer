...
from torchTraining.utils.registry import WEIGHT_INITIALIZERS, build_from_cfg
...
class PretrainedInit:
    ...
    def __call__(self, module):
        from torchTraining.runner.checkpoint import (
        ...
        if self.prefix is None:
            ...
            print_log(
                f"load {self.prefix} in model from: {self.checkpoint}", logger="current"
            )
...
def initialize(module, init_cfg):
    r"""根据初始化配置(可以是配置列表)初始化模型参数

    配置样例:
        >>> init_cfg = [dict(type='Normal', layer='Conv2d', name='conv', mean=0, std=1, bias=0), dict(type='Pretrained', checkpoint='*', prefix='*')]
        - type: 初始化方法名称, 预置选项包括: ``Constant``, ``Xavier``, ``Normal``, ``Uniform``, ``Kaiming``, ``Pretrained``.
        - layer: 初始化模型内特定类型层, 可以是 ``str`` 或 ``list[str]``.
        - name: 初始化模型内特定结构, 可以是 ``str``, 优先级比 layer 高.
        - 单个配置中, 当 ``type`` 为 ``Pretrained`` 时, 其内的 layer|name 参数失效.
        - 若有多个配置, 会按顺序依次初始化, 若有重复的层, 后面的配置会覆盖前面的配置.
        - 其余参数根据初始化方法不同而不同.

    Args:
        module (``torch.nn.Module``): the module will be initialized.
        init_cfg (dict | list[dict]): initialization configuration dict to
            define initializer. Has implemented 6 initializers
    ...
    for cfg in init_cfg:
        ...
        if "type" not in cp_cfg.keys():
            raise ValueError(f'Need "type" key, but got {cp_cfg}')

        if cp_cfg["type"] == "Pretrained":
            cp_cfg.pop("name", "")
            cp_cfg.pop("layer", "")

        name = cp_cfg.pop("name", "")

        if "layer" in cp_cfg.keys():
            cp_cfg["layer"] = (
                [cp_cfg["layer"]]
                if isinstance(cp_cfg["layer"], str)
                else cp_cfg["layer"]
            )
            assert isinstance(cp_cfg["layer"], list)

        func = build_from_cfg(cp_cfg, WEIGHT_INITIALIZERS)

        if "layer" in cp_cfg.keys():
            func.wholemodule = False
            func(module)
        elif name != "":
            func.wholemodule = True
            name_list = name.split(".")
            current_ = module
            for name in name_list:
                if hasattr(current_, name):
                    current_ = getattr(current_, name)
                else:
                    raise ValueError(f"{name} is not in {module}")
            func(current_)
        elif cp_cfg["type"] == "Pretrained":
            func(module)
        else:
            func.wholemodule = True
            func(module)
