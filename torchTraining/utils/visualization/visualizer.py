...
from torchTraining.config import Config
from torchTraining.utils.dist import master_only
from torchTraining.utils.logging import print_log
from torchTraining.utils.registry import VISBACKENDS, VISUALIZERS
from torchTraining.structures import BaseDataElement
from torchTraining.utils import ManagerMixin
from torchTraining.utils.visualization.utils import (
    check_type,
...
from torchTraining.utils.visualization.vis_backend import BaseVisBackend
...
class Visualizer(ManagerMixin):
    ...
    - 基本可视化方法
      ...
      - add_configs: 将配置写入所有可视化存储后端
      - add_graph: 将模型图写入所有可视化存储后端
      - add_image: 将图像写入所有可视化存储后端
      - add_scalar: 将标量写入所有可视化存储后端
    ...
    def __init__(
        ...
        if save_dir is None:
            ...
            names = [vis_backend.get("name", None) for vis_backend in vis_backends]
            if None in names:
                if len(set(names)) > 1:
                    raise RuntimeError(
                    ...
                    type_names = [vis_backend["type"] for vis_backend in vis_backends]
        ...
        (self.fig_save_canvas, self.fig_save, self.ax_save) = self._initialize_fig(
            fig_save_cfg
        )
    ...
    def get_backend(self, name) -> "BaseVisBackend":
        ...
        """
        return self._vis_backends.get(name)
    ...
    def draw_texts(
        ...
        font_properties: Optional[
            Union["FontProperties", List["FontProperties"]]
        ] = None,
    ) -> "Visualizer":
        ...
        check_type_and_length(
            "vertical_alignments", vertical_alignments, (str, list), num_text
        )
        ...
        if font_properties is None:
            font_properties = [None for _ in range(num_text)]
        else:
            check_type_and_length(
    ...
    def draw_binary_masks(
        ...
        assert img.shape[:2] == binary_masks.shape[1:], (
            "`binary_marks` must have " "the same shape with image"
        )
    ...
    def draw_featmap(
        ...
        assert featmap.ndim == 3, (
            f"Input dimension must be 3, " f"but got {featmap.ndim}"
        )
        ...
        if channel_reduction is not None:
            ...
            for i in range(topk):
                ...
                axes.imshow(
                    convert_overlay_heatmap(topk_featmap[i], overlaid_image, alpha)
                )
