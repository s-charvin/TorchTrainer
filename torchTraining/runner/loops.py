...
from torchTraining.utils.registry import LOOPS
...
class EpochBasedTrainLoop(BaseLoop):
    ...
    def run_iter(self, idx, data_batch: Sequence[dict]) -> None:
        ...
        # 这段代码的作用是启用梯度累积模式并在梯度累积过程中避免不必要的梯度同步. 它假设outputs是一个包含损失的字典.
...
class _InfiniteDataloaderIterator:
    ...
    recommended to use `torchTraining.dataset.InfiniteSampler` to enable the
    ...
    def __next__(self) -> Sequence[dict]:
        try:
            ...
            print_log(
                ...
                "recommended to use "
                "`torchTraining.dataset.InfiniteSampler` to enable the "
                "dataloader to iterate infinitely.",
                logger="current",
                level=logging.WARNING,
            )
