...
from torchTraining.utils.registry import FUNCTIONS
...
def default_collate(data_batch: Sequence) -> Any:
    ...
    if isinstance(data_item, (BaseDataElement, str, bytes)):
        ...
        # named_tuple
        return data_item_type(
            *(default_collate(samples) for samples in zip(*data_batch))
        )
    elif isinstance(data_item, Sequence):
        # check to make sure that the data_itements in batch have
        # consistent size
        ...
        return data_item_type(
            {key: default_collate([d[key] for d in data_batch]) for key in data_item}
        )
    else:
...
def input_collate(data_batch: Sequence) -> Any:
    ...
    def create_dict(data):
        if isinstance(data, Mapping):
            return {key: create_dict(data[key]) for key in data}
        else:
            return None

    result = create_dict(data_item)
