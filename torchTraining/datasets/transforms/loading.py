...
from torchTraining.utils.registry import TRANSFORMS
import torchTraining.utils.fileio as fileio
from torchTraining.utils import is_filepath, is_str
...
class LoadMultiChannelImageFromFiles(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        for name in results["img_path"]:
            try:
                img_bytes = fileio.get(name, backend_args=self.backend_args)
            except Exception as e:
                if self.ignore_empty:
                    return None
                else:
...
class LoadAudioFromFile(BaseTransform):
    ...
    def transform(self, results: dict) -> Optional[dict]:
        filename = results["audio_path"]
        try:
            audio_bytes = fileio.get(filename, backend_args=self.backend_args)
        ...
        assert audio is not None, f"failed to load audio: {filename}"
        if self.to_float32:
...
class LoadAudioFromFiles(BaseTransform):
    ...
    def transform(self, results: dict) -> Optional[dict]:
        ...
        audio = []
        sample_rate = []
        for filename in results["audio_path"]:
            filename = results["audio_path"]
            try:
                audio_bytes = fileio.get(filename, backend_args=self.backend_args)
            except Exception as e:
                if self.ignore_empty:
                    ...
                    raise e
            assert audio_bytes is not None, f"failed to load audio: {filename}"
        ...
        results["audio"] = audio
        results["sample_rate"] = sample_rate[0]
        return results
...
class LoadVideoFromFile(BaseTransform):
    ...
    def transform(self, results: dict) -> Optional[dict]:
        filename = results["video_path"]
        try:
            video_bytes = fileio.get(filename, backend_args=self.backend_args)
            video = videofrombytes(video_bytes, backend=self.decode_backend)
        except Exception as e:
            if self.ignore_empty:
                return None
            else:
        ...
        assert video is not None, f"failed to load video: {filename}"
        if self.to_float32:
